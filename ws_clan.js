function clan(){
	...
	if (LOCAL.getImperio().clan==siglas) {
		LOCAL.setClan(cargarClan(document));
	}
	...
	document.querySelectorAll(".lista1 tr").forEach(function(index, obj){
		if (index === 0) {
		    return;
		}

		var valor = parseInt(obj.children[2].innerHTML.replace(".", ""));
		...
		obj.children[2].insertAdjacentHTML("beforeend", "<span style='font-size: 11px'><b>" + mensaje + "</b></span>");
	})
}